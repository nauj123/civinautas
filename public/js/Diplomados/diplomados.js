...
/* 
	...
	function getGruposMediador() {
		$.ajax({
			...
			success: function(data) {
				informacion_diplomados = data;
				tabla_informacion_diplomados.clear().draw();
				informacion_diplomados.forEach((value, index) => {

					rowNode = tabla_informacion_diplomados.row.add([
						informacion_diplomados[index]["INSTITUCION"],
						informacion_diplomados[index]["NOMBREGRUPO"],
						informacion_diplomados[index]["MEDIADOR"],
						informacion_diplomados[index]["DOCENTE"],
						informacion_diplomados[index]["JORNADA"],
						"<center>"+informacion_diplomados[index]["ESTUDIANTES"]+"</center>",
						"<center><buton type='button' class='btn btn-danger retirar' data-id-estudiante='"+informacion_diplomados[index]["IDGRUPO"]+"' data-toggle='modal' data-target='#modal-editar-usuario'>Inactivar Grupo</buton></center>"
						]).draw().node();
				});
			},
			error: function(data){
